...
def update(rating, u_matrix, i_matrix, i, j,eta, delta, N_u, N_i):
    ...
    tmp=tmp_j/tmp_i
    for k in range(i_matrix.shape[1]):
        for f in range(i_matrix.shape[1]):
...
def pfp_KL(d_path,d_trfile,d_tefile,output,num_k,eta,delta,num_iter, N_u, N_i) :
    #    d_path=argv[1]
#    d_trfile=argv[2]
#    d_tefile=argv[3]
#    num_k=argv[4]
...
print num_k
print eta
pfp_KL(d_path,d_trfile,d_tefile,output,num_k,eta,delta,num_iter, N_u, N_i)
